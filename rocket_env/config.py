...
PRESETS: dict[str, dict[str, Any]] = {
    ...
    #
    # x_range(수평 퍼짐)는 그 자체로 난이도 축이다. 착륙장 반경이 50m이므로
    # ±50을 넘어서면 대부분의 에피소드가 옆으로 이동해야 하고, 옆이동에는
    # 기울임이 필요해 자세 축과 곱셈적으로 상호작용한다(합이 아니라 곱).
    # 그래서 라운드가 올라갈수록 단조 비감소(non-decreasing)하도록 둔다.
    # landing-basic과 landing-attitude는 일부러 ±50을 공유한다 — 2라운드가
    # 수평 퍼짐을 새로 올리지 않고 자세 축만 분리해서 시험하도록 하기 위해서다.
    "landing-basic": {
        "task": "landing",
