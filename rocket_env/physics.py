...
# 노즐 짐벌 한계. 실제 Falcon 9 는 ±5~7° 다. 짐벌 토크는 추력에 비례하는데
# (alpha = thrust·sin(phi)·6/H), ±20° 를 유지하면 최대 추력에서 각가속도가
# 과해져 기체가 스스로 텀블링한다 — 짐벌 권한은 추력과 함께 커진다는 점을
# 놓치면 안 된다. 최대 추력 2.5G·짐벌 8° 에서 각가속도는 약 23°/s² 로,
# 초기 각속도(최대 20°/s)를 1 초 안에 상쇄할 수 있는 여유가 있다.
...
# 최대 TWR 2.5. 실제 착륙 로켓(Falcon 9·Starship)의 착륙 연소 TWR 은
# 1.2~2 로, "무겁게 내려와 아슬아슬하게 감속"하는 저추력 자살연소다.
# 예전 최대 5G(순감속 4G)는 70 m/s 를 62 m 만에 세워, 빠르게 낙하하다
# 급브레이크처럼 꺾이는 비현실적 움직임을 만들었다. TWR 2.5 면 순감속
# 1.5G 라 같은 속도를 250 m 에 걸쳐 감속해, 감속 궤적이 낙하 구간 전체에
# 자연스럽게 퍼진다. 손 컨트롤러 착륙률은 12/20 -> 10/20 으로 거의 유지된다.
# 5단계. 각 단계가 서로 다른 일을 한다.
#   0      완전 무동력. 가장 빨리 떨어지지만 alpha = 0 이라 자세 제어 불가
#   0.6G   떨어지면서도 짐벌로 자세를 잡을 수 있는 저추력(호버 미만)
#   1.0G   중력과 정확히 상쇄 — 정지 호버. 접지 직전 미세 조정에 필수
#   1.6G   완만한 제동(순감속 0.6G)
#   2.5G   최대 제동(순감속 1.5G)
# 4단계로 줄이면 "빠르게 떨어지면서 자세 잡기"와 "호버" 중 하나를 잃는다.
THRUST_LEVELS = (0.0, 0.6 * G, 1.0 * G, 1.6 * G, 2.5 * G)
